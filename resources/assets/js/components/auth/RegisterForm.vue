<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  ...
  <div class="form-group has-feedback " :class="{ 'has-error': form.errors.has('firstname') }">
   <input type="text" class="form-control" :placeholder="trans('adminlte_lang_message.fullname')" name="firstname" value="" v-model="form.firstname" autofocus/>
   <span class="glyphicon glyphicon-user form-control-feedback"></span>
   <transition name="fade">
  ...
  <div class="g-recaptcha" data-sitekey="6LdlJVIUAAAAAM_utQevyWuVhDJ62Vw-xJuzO4D1">
  </div>
  <div class="row">
   <div class="col-xs-7">
    <label>
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        form: new Form({ username: '', firstname: '', lastname: '', email: '', password: '', password_confirmation: '', terms: '' })
      }
    },
    watch: {
